...
parser.add_argument("-b", "--basedirectory", help="the base directory for the data in the project")
...
runs_file = open(os.path.join(args.b,args.r),"r")
...
datasets_file = open(os.path.join(args.b,args.d),"w")

for run in runs:
    input_dir, experiment, replicate, genome, restriction_enzyme = run
    filenames = [j for j in os.listdir(os.path.join(args.b,input_dir)) if j.endswith(".hdf5") ]
    for fname in filenames:
        try:
            mydict = h5dict(os.path.join(args.b,input_dir,fname),'r')
        except:
            pass
